...
export default function EventTabItem({
  eventTabBageData,
  isSelected,
  onClick,
}: EventTabItemProps) {
  const imageUrl = isSelected
    ? eventTabBageData.tabBadgeImageUrl2
    : eventTabBageData.tabBadgeImageUrl1 //이미지가 string | undefined라 변수로 따로 저장

  return (
    <button
      className="w-[84px] h-[100px] flex flex-col justify-center items-center"
      onClick={onClick}
    >
      <span
      ...
      >
        {imageUrl ? (
          ...
          <span className={`${isSelected ? 'text-[#f8f8f8]' : 'text-black'}`}>
            {eventTabBageData.tabDescription}
          </span>
        )}
      </span>

      <span className="text-xs font-bold mt-1.5">
        {eventTabBageData.tabName}
      </span>
    </button>
  )
}
