...
export default function SignUpSelection() {
  const membershipDescriptions: membershipDescriptionType[] =
    membershipDescriptionData
    ...
    <div className="w-full px-[24px] pb-[60px] mt-[55px]">
      ...
      <ul className="mt-[24px] px-[16px] py-[20px] bg-[#f8f8f8]">
        {membershipDescriptions.map((item) => (
          <li key={item.id} className="list-item leading-[22px] ">
            ...
            <p className="text-[14px] mt-[6px] leading-[20px] tracking-[-0.3px] text-[#787878]">
              {item.description}
            </p>
          </li>
        ))}
      ...
      <div className="mt-[12px]  flex flex-row gap-[8px] font-[400] tracking-[-1px] ">
        <Link href={'/'} className="w-full ">
          <Button
            variant={'outline'}
            className="py-[6px] w-full h-full text-[12px]  font-[400] "
          >
            회원등급 혜택 자세히보기
          </Button>
        </Link>
